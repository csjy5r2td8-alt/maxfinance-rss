...
# com a bandeira do pais/area (para o cabecalho de cada bloco).
COMPETICOES = [
    ("PPL", "Liga Portugal", "https://crests.football-data.org/765.svg"),
    ("PL", "Premier League", "https://crests.football-data.org/770.svg"),
    ("PD", "La Liga", "https://crests.football-data.org/760.svg"),
    ("SA", "Serie A", "https://crests.football-data.org/784.svg"),
    ("BL1", "Bundesliga", "https://crests.football-data.org/759.svg"),
    ("FL1", "Ligue 1", "https://crests.football-data.org/773.svg"),
    ("CL", "Champions League", "https://crests.football-data.org/EUR.svg"),
    ("EC", "Europeu", "https://crests.football-data.org/EUR.svg"),
    ("WC", "Mundial", None),
]
...
def linha_resultado(m):
    ...
    eh = escudo_img(m["homeTeam"].get("crest"))
    ea = escudo_img(m["awayTeam"].get("crest"))
    hs = m["score"]["fullTime"]["home"]
    as_ = m["score"]["fullTime"]["away"]
    return f"""<tr>
      <td style="padding:6px 8px;border-bottom:1px solid #eee;">{eh}{h}</td>
      <td style="padding:6px 8px;text-align:center;border-bottom:1px solid #eee;font-weight:700;">{hs} - {as_}</td>
      <td style="padding:6px 8px;border-bottom:1px solid #eee;">{ea}{a}</td>
    </tr>"""
...
def bloco_competicao(nome, bandeira, resultados, hoje_jogos, tabela, golos):
    bandeira_img = escudo_img(bandeira, 20) if bandeira else ""
    partes = [f"""<h2 style="font-size:16px;color:#1f2d3d;margin:26px 0 10px;
      border-bottom:2px solid #eee;padding-bottom:6px;">{bandeira_img}{nome}</h2>"""]

    # 1) Resultados de ontem
    if resultados:
    ...
    if tabela:
        linhas = "".join(f"""<tr>
          ...
          <td style="padding:5px 8px;border-bottom:1px solid #eee;">{escudo_img(r['escudo'], 16)}{r['equipa']}</td>
    ...
    if golos:
        linhas = "".join(f"""<tr>
          ...
          <td style="padding:5px 8px;border-bottom:1px solid #eee;color:#666;">{escudo_img(g['escudo'], 16)}{g['equipa']}</td>
...
def main():
    ...
    for codigo, nome, bandeira in COMPETICOES:
        ...
        blocos.append(bloco_competicao(nome, bandeira, resultados, hoje_jogos, tabela, golos))
...
<body style="margin:0;padding:0;background:#f0f2f5;font-family:Arial,Helvetica,sans-serif;color:#222;">
  <div style="max-width:600px;margin:0 auto;padding:24px;">
    ...
    <p style="color:#666;margin:0 0 6px;font-size:14px;">
      Resultados de ontem, jogos de hoje, classificações e marcadores.
    </p>
