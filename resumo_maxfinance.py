...
def get_blog():
    try:
        ...
        def container_de(a_tag, profundidade=6):
            ...
            'caixinha' (div, article, li...) que tambem contem o titulo
            (h2/h3). Isto evita sair para o resto da pagina (menus, etc)."""
            node = a_tag
        ...
        # Ancora-se nos links de artigo (unico sinal fiavel), nunca nos titulos.
        for a in soup.find_all("a", href=True):
            ...
            for node in container.find_all(True):
                if node is h or node in h.descendants:
                    continue
